...
os.environ["DEEPFOLD_FORCE_TUNE"] = "1"
...
from deepfold.ops.utils import sample_tensors
...
def valid_dtypes(value: str) -> list[torch.dtype]:
    ...
    if invalid_dtypes:
        raise argparse.ArgumentTypeError(f"Invalid dtype(s): {invalid_dtypes}. " f"Allowed values are: {allowed_dtypes}")
...
def parse_int_list(value: str) -> list[int]:
    try:
        ...
        raise argparse.ArgumentTypeError('Invalid format. Please provide comma-separated integers (e.g., "256,512,1024")')
        ...
        for h in heads:
            for n in ns:
                for dtype in dtypes:
                    ...
                    q, k, v, bias, mask = sample_tensors(n, d, h, use_mask=True, device=torch.device("cuda"), dtype=dtype, std=1.0)
...
def main():
    parser = argparse.ArgumentParser(description="Autotune Triton kernels over these parameters.")

    parser.add_argument("--min-n", type=int, help="Miniumum sequence length to tune.", default=16)

    parser.add_argument("--max-n", type=int, help="Maximum sequence length to tune.", default=1024)
