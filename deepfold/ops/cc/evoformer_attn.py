...
def _attention_fwd(Q, K, V, bias1, bias2):
    assert Q.shape[-3] > 16, "seq_len must be greater than 16"
    O = torch.empty_like(Q, dtype=Q.dtype)
    assert Q.get_device() == torch.cuda.current_device()
    assert K.get_device() == torch.cuda.current_device()
    assert V.get_device() == torch.cuda.current_device()
    assert bias1.get_device() == torch.cuda.current_device()
    assert bias2.get_device() == torch.cuda.current_device()
    nh = Q.shape[-2]
    nq = (Q.shape[-3] + 31) // 32 * 32
    nb = np.prod(Q.shape[:-3])
    lse = torch.empty((nb, nh, nq), dtype=torch.float32, device=Q.device)
    op.evoformer_attention_fwd(Q, K, V, bias1, bias2, O, lse)
    return O, lse


def _attention_bwd(dO, Q, K, V, O, lse, bias1, bias2, bias1_grad, bias2_grad):
    ...
    assert dO.get_device() == torch.cuda.current_device()
    assert Q.get_device() == torch.cuda.current_device()
    assert K.get_device() == torch.cuda.current_device()
    assert V.get_device() == torch.cuda.current_device()
    assert O.get_device() == torch.cuda.current_device()
    ...
    op.evoformer_attention_bwd(dO, Q, K, V, O, lse, delta, bias1, bias2, dQ, dK, dV, dB1, dB2)
...
class EvoformerFusedAttention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, bias1=None, bias2=None):
        ...
        k = k.contiguous()
        v = v.contiguous()
        o, lse = _attention_fwd(q, k, v, bias1_, bias2_)
    ...
    def backward(ctx, grad_output):
        ...
        dQ, dK, dV, dB1, dB2 = _attention_bwd(grad_output, q, k, v, o, lse, bias1, bias2, is_b1_grad, is_b2_grad)
        if not is_b1_grad:
            dB1 = None
        if not is_b2_grad:
            dB2 = None
...
def DS4Sci_EvoformerAttention(Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, biases: List[torch.Tensor]):
    assert len(biases) <= 2
